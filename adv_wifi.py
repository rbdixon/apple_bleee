...
parser.add_argument(
    "-p", "--phone", default="none", help="Phone number (example: 39217XXX514)"
)
parser.add_argument(
    "-e", "--email", default="none", help="Email address (example: test@test.com)"
)
parser.add_argument(
    "-a", "--appleid", default="none", help="Email address (example: test@icloud.com)"
)
...
def get_hash(data, size=3):
    return tuple(
        bytearray.fromhex(hashlib.sha256(data.encode("utf-8")).hexdigest())[:size]
    )
